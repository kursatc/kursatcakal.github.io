"use strict";
var jsToTopButton = $('#toTopButton');

$(window).scroll(function() {
  if ($(window).scrollTop() > 300) 
  {
    jsToTopButton.addClass('show');
  } else 
  {
    jsToTopButton.removeClass('show');
  }
});


jsToTopButton.on('click', function(e) {
  e.preventDefault();
	...
	if ($('#front-slider').length > 0) {
		$('#front-slider').fractionSlider({
			...
			'startCallback': function(el, currentSlide, lastSlide, step ){
				$('.topslider .slider .prev').text('').appendTo('.topslider');
				
				$('.topslider .slider .next').text('').appendTo('.topslider');
